...
if(isset($_POST["send"])){
	...
	$sql = "SELECT id FROM users WHERE email='$e' AND is_paid='1' LIMIT 1";
	$query = mysqli_query($db_conx, $sql); 
	...
	if ($e_check > 0)
	{
		...
		$ExparxId= 'SIXTHSENSE-'.rand(10000,99999);
		$sql = "update users set sixthsenseID='".$ExparxId."' where email='".$e."'";
		$query = mysqli_query($db_conx, $sql);
		$sql2 = "update tbl_sixthsense set sixthsenseID='".$ExparxId."',is_paid='1' where uid='".$_SESSION['userid']."'";
		mysqli_query($db_conx, $sql2);
                $to = "$e";
		
		$from = "info@esparx2k16.com";
		$subject = 'Esparx2k16:: Sixth Sense ID created Successfull';
		$title="Sixth Sense ID Created";
		$heading='Please check your Sixth Sense Id:';
		
		
		
		$message = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Esparx2K16 Message</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;"><div style="padding:10px; background:#333; font-size:24px; color:#CCC;">'.$title.'</div><div style="padding:24px; font-size:17px;">Dear User,<br /><br/><b>Thank you, you have done your payment successfully.</b><br/>'.$heading.'<b>'.$ExparxId.'</b><br/><br/>please write to<u>info@esparx2k16.com</u> if you experience any technical difficulties.</div></body></html>';
		$headers = "From: $from\n";
		$headers .= "MIME-Version: 1.0\n";
		$headers .= "Content-type: text/html; charset=iso-8859-1\n";
		mail($to, $subject, $message, $headers); 
		header("location:successsixthsensepayment.php?id=success");
		
	}
	else
	{
	header("location:successsixthsensepayment.php?id=err");
        exit();
	}
	exit();
}
...
<div class="twitter" id="Partnershipd" style="background-color: #fff">
  <div class="container" style="background-color:#fff;margin-top: 40px;border: 1px solid #DDDDDD">
   ...
   <br>
	...
	{
	echo "<span style=color:green;>You have successfully registered for Sixth Sense. An email has been sent with Sixth Sense to your email id.";
	}
...
<?php if(isset($_GET['id']) && $_GET['id']=='err' || !isset($_GET['id']))
{  ?>
<form method="post">
<div class="twitter_left2" style="text-align:center">
	<p><b>Note:</b><span style=background-color:yellow;> Please enter your Userid which you used at the time of signup, We will send you a Sixth Sense Transaction number to this email id.</span></p><br>
</div>
<div class="twitter_left2" style="text-align:center">
