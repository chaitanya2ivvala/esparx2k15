...
if(isset($_POST["send"])){
	...
	$sql = "SELECT id FROM users WHERE email='$e' AND is_paid='0' LIMIT 1";
	$query = mysqli_query($db_conx, $sql); 
	$e_check = mysqli_num_rows($query);
	if ($e_check > 0)
	{
		$ExparxId= 'Esparx-Reg-'.rand(10000,99999);
		$sql = "update users set Rexparxid='".$ExparxId."',is_paid='1',activated='1' where email='".$e."'";
		$query = mysqli_query($db_conx, $sql);
		
                $to = "$e";
		$from = "info@esparx2k16.com";
		$subject = 'Esparx2k16 Account Activation Successfull';
		$message = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Esparx2K16 Message</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;"><div style="padding:10px; background:#333; font-size:24px; color:#CCC;">Esparx2K16 Account Successfull</div><div style="padding:24px; font-size:17px;">Dear User,<br /><br/><b>Thank you, you have done your payment successfully.</b><br/> Please check your Exprax Id:<b>'.$ExparxId.'</b><br/><br />After you receive the payment confirmation to your email, you can successfully login using your<br /> E-mail Address: <b>'.$e.'</b><br>Password: *****<br/><br/>please write to<u>info@esparx2k16.com</u> if you experience any technical difficulties.</div></body></html>';
		$headers = "From: $from\n";
		$headers .= "MIME-Version: 1.0\n";
		$headers .= "Content-type: text/html; charset=iso-8859-1\n";
		mail($to, $subject, $message, $headers); 
		header("location:registration_successfull.php?id=success");
		//$m="An email has been sent with Esparxid to your email id.<br/> Now You can login into your account.";
		//exit();
	}
	else
	{
	header("location:registration_successfull.php?id=err");
        exit();
	}
	exit();
}
...
<div class="twitter" id="Partnershipd" style="background-color: #fff">
  <div class="container" style="background-color:#fff;margin-top: 40px;border: 1px solid #DDDDDD">
    <div class="twitter_left2"></br>
      <center><h3 class="m_21" style="color: #fff;font-family: verdana;text-transform: none;background-color: #4BCAFF;padding: 10px;">Payment Successfull</h3></center>
   ...
   <br>
	...
	{
	echo "<span style=color:green;>Your account has been activated successfully. An email has been sent with Esparxid to your email id.<br/> Now You can login into your account.";
	}
	else if(isset($_GET['id']) && $_GET['id']=='err')
	{
	echo "<span style=color:red;>YOpps. Either you have already activated your account OR the entered email is incorrect";	
	}?></p>
	</div></br>
<form method="post">
<div class="twitter_left2" style="text-align:center">
	<p><b>Note:</b><span style=background-color:yellow;> Please enter your valid email id which you used at the time of signup, We will send you a Transaction number to this email id.</span></p><br>
</div>
<div class="twitter_left2" style="text-align:center">
...
<br>
<div class="twitter_left2" style="text-align:center">
      <input type="submit"  name="send" value="Click to Submit " style="background-color:#1CB4E5;padding: 10px;">
 </div>
</form>
</br>
</div>
