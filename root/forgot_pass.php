...
if(isset($_POST["e"])){
	...
	$sql = "SELECT id, fname FROM users WHERE email='$e' AND activated='1' LIMIT 1";
	...
	if($numrows > 0){
		...
		$sql = "UPDATE useroptions SET temp_pass='$hashTempPass' WHERE fname='$f' LIMIT 1";
	    $query = mysqli_query($db_conx, $sql);
		$to = "$e";
		...
		$msg = '<h2>Hello '.$f.'</h2><p>You indicated that you forgot your login password. We can generate a temporary password for you to log in with, then once logged in you can change your password to anything you like.</p><p>After you click the link below your password to login will be:<br /><b>'.$tempPass.'</b></p><p><a href="http://www.esparx2k16.com/forgot_pass.php?f='.$f.'&p1='.$hashTempPass.'">Click here now to apply the temporary password shown below to your account</a></p><p>If you do not click the link in this email, no changes will be made to your account. In order to set your login password to the temporary password you must click the link above.</p>';
...
if(isset($_GET['f']) && isset($_GET['p1'])){
	$f = preg_replace('#[^a-z0-9]#i', '', $_GET['f']);
	$temppasshash = preg_replace('#[^a-z0-9]#i', '', $_GET['p1']);
	if(strlen($temppasshash) < 10){
		exit();
	}
	$sql = "SELECT id FROM useroptions WHERE fname='$f' AND temp_pass='$temppasshash' LIMIT 1";
	$query = mysqli_query($db_conx, $sql);
	$numrows = mysqli_num_rows($query);
	if($numrows == 0){
		...
		$id = $row[0];
		$sql = "UPDATE users SET password='$temppasshash' WHERE id='$id' AND fname='$f' LIMIT 1";
	    $query = mysqli_query($db_conx, $sql);
		$sql = "UPDATE useroptions SET temp_pass='' WHERE id='$id' AND fname='$f' LIMIT 1";
	    $query = mysqli_query($db_conx, $sql);
